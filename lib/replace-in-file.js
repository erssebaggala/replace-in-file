...
const replaceStream = require('./helpers/replace-stream');
...
* Stream Sync API
*/
replaceInFile.stream = function(config, cb) {
  ...
  return getPathsAsync(files, ignore, disableGlobs, allowEmptyPaths, glob)
    ...
    .then(paths => Promise.all(paths.map(file => {
      ...
      return replaceStream(file, from, to, encoding, dry);
    })))

    //Convert results to array of changed files
    .then(results => {
      return results
        .filter(result => result.touched)
        .map(result => result.file);
    })
