...
describe('Replace in file', function() {
  ...
  describe('Sync', () => {
   ...
   describe('Stream', function () {
    ...
    it('should throw an error when no config provided', () => {
      return expect(replace.stream()).to.eventually.be.rejectedWith(Error);
    });

    it('should throw an error when invalid config provided', () => {
      return expect(replace.stream(42)).to.eventually.be.rejectedWith(Error);
    });

    it('should throw an error when no `files` defined', () => {
      return expect(replace.stream({
        from: /re\splace/g,
        to: 'b',
      })).to.eventually.be.rejectedWith(Error);
    });
